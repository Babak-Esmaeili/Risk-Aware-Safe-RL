 function [x, u, P_1_inv, P_2_inv, P_3_inv, P_LQR_DD_2_opt_max, cost_average, t] = DD_safe_optimal_controller_3(N,U0,x0_id,x0_sim,lambda,delta,eta,sigma,noise)
...
%     U0_B = zeros(nInputs,N);
% %     U0_B(:,1:N) = 5*randn(nInputs,N);
    
%     for i = 1:N
%     
%         rand_signal_B = rand(nInputs,1);
%     
%         if(round(rand_signal_B(1))==1)
%     
%             U0_B(1,i) = 1;
%     
%         else
...
%     end
    ...
    v_h_all = [];
    G_K_opt_all = {G_K_1_opt,G_K_2_opt,G_K_3_opt};
    ...
    G_K_opt_all_partiotions = cell(1,nPartitions);
    ...
    for iii = 1:nPartitions
        ...
        if(label_v_h_all_accepted(iii) == label_v_h_all_accepted(iii+1))
            ...
            G_K_opt_all_partiotions{iii} = G_K_opt_all{label_v_h_all_accepted(iii)};
            K_opt_all_partiotions(iii,:) = K_opt_all{label_v_h_all_accepted(iii)};
    
        else
    
            G_K_opt_all_partiotions{iii} = [G_K_opt_all{label_v_h_all_accepted(iii)}*v_h_all_accepted(:,iii) G_K_opt_all{label_v_h_all_accepted(iii+1)}*v_h_all_accepted(:,iii+1)]*([v_h_all_accepted(:,iii) v_h_all_accepted(:,iii+1)]^-1);
...
%     nu = zeros(1,nSteps);
    ...
    alpha_interpolation_vec = zeros(nRealization,nSteps);
    ...
    for ii = 1:nRealization
        ...
        for k = 1:nSteps
            ...
            G_K_curr = G_K_opt_all_partiotions{idx};
...
%             end
            ...
            u_safe = K_safe*x(nStates*ii-(nStates-1):nStates*ii,k);

            u_opt = K_LQR_DD_2*x(nStates*ii-(nStates-1):nStates*ii,k);

            x_next_opt = X1*G_K_curr*x(nStates*ii-(nStates-1):nStates*ii,k) + B_hat*(u_opt-u_safe);

            if(F_CH*x_next_opt<=eta*g_CH)

                u(:,k) = u_opt;

            else

                fprintf('Safety violation is going to occur! Using the safe backup controller at sample #%d of realization #%d...\n',k,ii);
                violation_counter = violation_counter + 1;

                alpha_interpolation = sdpvar(1,1);

                cost_interpolation = alpha_interpolation;

                x_next_safe_opt = X1*G_K_curr*x(nStates*ii-(nStates-1):nStates*ii,k) + (1-alpha_interpolation)*B_hat*(u_opt-u_safe);

                const_1 = (F_CH*x_next_safe_opt) <= eta*g_CH;
                const_2 = alpha_interpolation >= 0;
                const_3 = alpha_interpolation <= 1;

                constraints_interpolation = [const_1; const_2; const_3];

                ops = sdpsettings('solver','mosek','verbose',0);
                diagnostics = optimize(constraints_interpolation, cost_interpolation, ops);

                if(diagnostics.problem == 0)

                    alpha_interpolation_opt = value(alpha_interpolation);

                    alpha_interpolation_vec(ii,k) = alpha_interpolation_opt;

                    disp(['alpha_interpolation = ' num2str(alpha_interpolation_opt)]);
                ...
                u(:,k) = alpha_interpolation_opt*u_safe + (1-alpha_interpolation_opt)*u_opt;
